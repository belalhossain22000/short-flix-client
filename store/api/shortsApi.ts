...
export const shortsApi = createApi({
  ...
  tagTypes: ["Shorts"],
  endpoints: (builder) => ({
    getShorts: builder.query<ShortsResponse, { page?: number; limit?: number; search?: string; tag?: string }>({
    ...
    addShort: builder.mutation<Short, { title: string; videoUrl: string; tags: string[] }>({
      query: (body) => ({
        url: "/shorts",
        method: "POST",
        body,
